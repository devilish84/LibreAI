#ifndef INCLUDED_COM_SUN_STAR_CONTAINER_XCONTAINERQUERY_HPP
#define INCLUDED_COM_SUN_STAR_CONTAINER_XCONTAINERQUERY_HPP

#include "sal/config.h"

#include "com/sun/star/container/XContainerQuery.hdl"

#include "com/sun/star/beans/NamedValue.hpp"
#include "com/sun/star/container/XEnumeration.hpp"
#include "com/sun/star/uno/XInterface.hpp"
#include "com/sun/star/uno/Reference.hxx"
#include "com/sun/star/uno/Sequence.hxx"
#include "com/sun/star/uno/Type.hxx"
#include "cppu/unotype.hxx"
#include "rtl/ustring.hxx"

namespace com { namespace sun { namespace star { namespace container {

inline ::css::uno::Type const & cppu_detail_getUnoType(SAL_UNUSED_PARAMETER ::css::container::XContainerQuery const *) {
    static typelib_TypeDescriptionReference * the_type = 0;
    if ( !the_type )
    {
        typelib_static_mi_interface_type_init( &the_type, "com.sun.star.container.XContainerQuery", 0, 0 );
    }
    return * reinterpret_cast< ::css::uno::Type * >( &the_type );
}

} } } }

SAL_DEPRECATED("use cppu::UnoType") inline ::css::uno::Type const & SAL_CALL getCppuType(SAL_UNUSED_PARAMETER ::css::uno::Reference< ::css::container::XContainerQuery > const *) {
    return ::cppu::UnoType< ::css::uno::Reference< ::css::container::XContainerQuery > >::get();
}

::css::uno::Type const & ::css::container::XContainerQuery::static_type(SAL_UNUSED_PARAMETER void *) {
    return ::cppu::UnoType< ::css::container::XContainerQuery >::get();
}

#endif // INCLUDED_COM_SUN_STAR_CONTAINER_XCONTAINERQUERY_HPP
